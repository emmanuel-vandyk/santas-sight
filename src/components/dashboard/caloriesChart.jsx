...
export const CaloriesChart = ({ data }) => {
  const chartData = [
    ...
    { name: 'Available', value: (data.totalCookies - data.consumedCookies) * 100 },
  ];
  ...
  return (
    <Card className="col-span-1">
      ...
      <CardContent>
        ...
        <Badge variant="secondary" className="mb-2">Cookies: {data.consumedCookies} / {data.totalCookies}</Badge>
        <ResponsiveContainer width="100%" height={200}>
          <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            ...
            >
              {chartData.map((index) => (
                <Cell key={`cell-${index}`} fill={index === 0 ? 'gray' : '#4d7c0f'} />
              ))}
            </Pie>
